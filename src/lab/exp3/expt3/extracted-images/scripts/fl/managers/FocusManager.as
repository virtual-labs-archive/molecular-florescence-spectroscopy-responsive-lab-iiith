   ...
   public class FocusManager implements IFocusManager
   {
      ...
      private function addFocusables(param1:DisplayObject, param2:Boolean = false) : void
      {
         ...
         if(o is DisplayObjectContainer)
         {
            ...
            if(doc is Stage || docParent is Stage || Boolean(doc.tabChildren))
            {
               i = 0;
      ...
      private function tabEnabledChangeHandler(param1:Event) : void
      {
         ...
         {
            if(!_loc3_ && Boolean(isTabVisible(_loc2_)))
            {
      ...
      private function getTopLevelFocusTarget(param1:InteractiveObject) : InteractiveObject
      {
         ...
         try
         {
            while(true)
            {
               ...
               {
                  if(o is IFocusManagerComponent && Boolean(IFocusManagerComponent(o).focusEnabled) && Boolean(IFocusManagerComponent(o).mouseFocusEnabled) && Boolean(UIComponent(o).enabled))
                  {
                     break;
                  }
                  o = o.parent;
